...
class Test:
    ...
    def test(self):
        ...
        if self.bn:
            name = '~/Domain_Adaptation/configs/ry.deeplab.coral.shanghai/train_log/test_bn.json'
        else:
            name = '~/Domain_Adaptation/configs/ry.deeplab.coral.shanghai/train_log/test.json'

        with open(name, 'w') as f:
            json.dump(res, f)

    def get_neck_feat(self, dataloader, trainloader, city):
        '''
    ...
    def test_neck_coral(self):
        ...
        for i, city in enumerate(self.target):
            ...
            res[city] = {'Acc': tA[i], 'IoU': tI[i], 'mIoU': tIm[i]}
        if self.bn:
            with open('~/Domain_Adaptation/configs/ry.deeplab.coral.paris/train log/test_bn.json', 'w') as f:
                json.dump(res, f)
        else:
            with open('~/Domain_Adaptation/configs/ry.deeplab.coral.paris/train log/test.json', 'w') as f:
                json.dump(res, f)
