...
class Trainer(object):
    ...
    def training(self, epoch):
        ...
        for i, sample in enumerate(tbar):
            ...
            # print(output.shape)
            # print(self.argmax.apply(output).shape)
            D_out = self.model_D(self.argmax.apply(output))
            loss2 = self.criterion_D(D_out, Variable(torch.FloatTensor(D_out.data.size()).fill_(self.gt_label)).cuda())
            # loss2.backward()
            # self.optimizer_D.step()
            # adv_loss += self.config.beta * loss2.item()

            total_loss = loss1 + self.config.beta * loss2
            total_loss.backward()
            self.optimizer.step()
            seg_loss += loss1.item()
            self.optimizer_D.step()
            ...
            loss_D1 = self.criterion_D(D_out1, Variable(torch.FloatTensor(D_out1.data.size()).fill_(self.prediction_label)).cuda())
            ...
            target_c0 = (1 - target)
            target_c1 = target
            target = torch.stack([target_c0, target_c1], dim=1)
            # print(target.shape)
            D_out2 = self.model_D(target)
            loss_D2 = self.criterion_D(D_out2, Variable(torch.FloatTensor(D_out2.data.size()).fill_(self.gt_label)).cuda())
        ...
        self.vis.line(X=torch.tensor([epoch]), Y=torch.tensor([seg_loss+adv_loss]), win='loss', name='train',
                      opts=dict(title='loss', xlabel='epoch', ylabel='loss'),
                      update='append' if epoch > 0 else None)
        self.vis.line(X=torch.tensor([epoch]), Y=torch.tensor([seg_loss]), win='seperate_loss', name='seg',
                      opts=dict(title='seperate_loss', xlabel='epoch', ylabel='loss'),
                      update='append' if epoch > 0 else None)
        self.vis.line(X=torch.tensor([epoch]), Y=torch.tensor([adv_loss]), win='seperate_loss', name='adv',
                      update='append')
        self.vis.line(X=torch.tensor([epoch]), Y=torch.tensor([D_loss]), win='D_loss',
                      opts=dict(title='D_loss', xlabel='epoch', ylabel='loss'),
                      update='append' if epoch > 0 else None)
    ...
    def validation(self, epoch):
        ...
        for i, sample in enumerate(tbar):
            ...
            output = output.detach()
            D_out = self.model_D(self.argmax.apply(output))
            adv_loss = self.criterion_D(D_out, Variable(torch.FloatTensor(D_out.data.size()).fill_(self.gt_label)).cuda())
        ...
        self.vis.line(X=torch.tensor([epoch]), Y=torch.tensor([seg_loss+adv_loss]), win='loss', name='val',
                      update='append')
...
def main():
    ...
    parser.add_argument('--resume', type=str,
                        default='/usr/xtmp/satellite/train_models/xh.deeplab.resnet.paris/epoch3812.pth',
