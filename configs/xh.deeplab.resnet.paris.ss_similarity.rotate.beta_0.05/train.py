...
class Trainer(object):
    def __init__(self, config, args):
        ...
        # criterion for image output and transformed image output
    ...
    def training(self, epoch):
        ...
        for i, sample in enumerate(tbar):
            ...
            # transform image
            transformer = Transformer(self.args.cuda)
            image_t, target_t = transformer(image), transformer(target)
            ...
            output_t = self.model(image_t)
            loss1 = self.criterion(output, target)
            loss2 = self.criterion(output_t, target_t)
            # transform the output segmentation
            output_tt = transformer(output)
            loss3 = self.criterion_t(output_tt, output_t)
            ...
            loss_1 += loss1.item()
            loss_2 += loss2.item()
            loss_transform += (config.beta * loss3).item()
        ...
        self.vis.line(X=torch.tensor([epoch]), Y=torch.tensor([loss_1]), win='seperate_train_loss', name='loss1',
                      opts=dict(title='seperate_train_loss', xlabel='epoch', ylabel='loss'),
                      update='append' if epoch > 0 else None)
        self.vis.line(X=torch.tensor([epoch]), Y=torch.tensor([loss_2]), win='seperate_train_loss', name='loss2',
                      update='append')
        self.vis.line(X=torch.tensor([epoch]), Y=torch.tensor([loss_transform]), win='seperate_train_loss', name='loss_transform',
                      update='append')
